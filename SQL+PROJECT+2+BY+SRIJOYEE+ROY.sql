...
FROM film
WHERE
     last_update >= DATE_SUB('2005-05-05 22:04:30', INTERVAL 30 DAY);
...
SELECT f.title AS movie_name, f.length AS movie_duration
FROM film f
LEFT JOIN
    inventory i USING (film_id)
WHERE
...
SELECT GROUP_CONCAT(CONCAT(a.first_name, ' ', a.last_name)) AS actor_name,
GROUP_CONCAT(a.actor_id) actor_ids, f.description
FROM actor a
JOIN
...
SELECT CONCAT_WS('  ', c.first_name, c.last_name) AS customer_full_name, i.film_id,
COUNT(*) AS rental_count
FROM customer c
JOIN
...
WHERE
    c.name = 'comedy'
	AND f.rental_rate > (SELECT AVG(rental_rate)
FROM film)
;
...
SELECT c.customer_id, CONCAT_WS('  ', c.first_name, c.last_name) AS customer_full_name,
...
# related to the "rental" table. [Hint: using Information_schema] 
...
SELECT s.staff_id, CONCAT(c.city, ',', co.country) AS store_location,
SUM(p.amount) AS total_revenue
FROM staff s
JOIN
...
SELECT r.rental_date AS visiting_day, CONCAT(c.first_name, ' ', c.last_name) AS customer_name,
f.title AS film_title,
DATEDIFF(r.return_date, r.rental_date) AS no_of_rental_days, p.amount AS amount_paid,
(p.amount / (SELECT SUM(amount)
FROM payment
WHERE
...
p.amount AS payment_amount, total_rental_cost,
CASE
...
LEFT JOIN
    (SELECT r.rental_id, SUM(f.rental_rate) AS total_rental_cost
    FROM rental r
